...
analisis <- function(lambda, v1, v2, mu1, mu2, K, beta, sigma, nsteps) {
    ...
    # El punto es:
    p0 = c(x0, 0)
    lineas = paste0("Punto de equilibrio libre de infección (x, y): ", toString(round(p0, 2)), "\n\n")
    ...
    g = ggplot(d, aes(x = x, y = valor, group = funcion, color = funcion)) +
        ...
        theme_bw() +
        ggtitle("Gráfico de las funciones f1 y f2",
                subtitle = "La o las intersecciones entre ambas indican la coordenada en x de los puntos de equilibrio con infección crónica.")
    ...
    if (verifican) {
        ...
        lineas = paste0(lineas, "Se verifican las condiciones 3.6 y 3.7 (ver Sobre el modelo)\nPor lo tanto, podemos encontrar los puntos de equilibrio restantes y clasificarlos.\n\n")
        ...
        for (i in 1:nrow(Pestrella)) {
            lineas = paste0(lineas, "Punto de equilibrio con infección crónica (x, y) = ", toString(round(Pestrella[i, ], 2)), "\n\n")
        }
        ...
        lineas = paste0(lineas, "sigma_0 = ", round(sigma0, 4), "\n")
        lineas = paste0(lineas, "sigma_c = ", round(sigmac, 4), "\n")
        ...
        lineas = paste0(lineas, "R0_sigma = ", round(r0_sigma, 4), "\n")
        ...
        if (0 < r0_sigma & r0_sigma < r0_sigma0) {
            ...
            lineas = paste0(lineas, "R0(sigma0) < R0(sigma) < 1 --> el sistema tiene dos puntos de atracción, un punto de equilibrio libre de infección y otro de infección crónica, más un punto de equilibrio inestable (punto de silla).\n")
        } else {
        ...
        lineas = paste0(lineas, "No se verifican las condiciones 3.6 y 3.7 (ver Sobre el Modelo)\n No hay otros puntos de equilibrio.\n\n")
