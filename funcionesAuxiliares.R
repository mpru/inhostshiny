...
analisis <- function(lambda, v1, v2, mu1, mu2, K, beta, sigma, nsteps) {
    ...
    # El punto es:
    p0 = c(x0, 0)
    lineas = paste0("Punto de equilibrio libre de infección: ", toString(round(p0, 2)), "\n\n")
    ...
    if (verifican) {
        ...
        if (0 < r0_sigma & r0_sigma < r0_sigma0) {
            ...
            lineas = paste0(lineas, "R0(sigma0) < R0(sigma) < 1 --> el sistema tiene dos puntos de atracción, el punto de equilibrio libre de infección y uno de infección crónica, y otro punto infección crónica que es un punto de silla.\n")
        } else {
        ...
        lineas = paste0(lineas, "No se verifican las condiciones 3.6 y 3.7 (ver notas)\n No se presenta el análisis.\n\n")
